...
function History() {
  const [topics, setTopics] = useState([]); // setup a state for topic
  ...
  useEffect(() => {
    // window.innerWidth works initially, but React won't re-render when the window is resized.
  ...
  useEffect(() => {
    const myNotes = async () => {
      ...
      try {
        ...
        setTopics(Array.isArray(res.data) ? res.data : []); // check if it is a array, and set in topics if it is
      } catch (error) {
        console.log(error);
      }
    };
  ...
  const openNotes = async (noteId) => { // Fetch note
     setLoading(true)
     setActiveNodeId(noteId)
    try {
      const res = await axios.get(serverUrl + `/api/notes/${noteId}`, {withCredentials:true}) // sending noteId with url
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 px-6 py-8">
      <motion.header
        ...
        className="mb-10 rounded-2xl bg-black/80 backdrop-blur-xl border border-white/10 px-8 py-6 items-start flex justify-between 
      md:items-center gap-4 flex-wrap shadow-[0_20px_45px_rgba(0,0,0,1.6)]"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <AnimatePresence>
          {isSidebarOpen && (
            ...
            h-full lg:h-[75vh] lg:col-span-1 bg-black/90 lg:bg-black/80
            backdrop-blur-xl border border-white/10
            shadow-[0_20px_45px_rgba(0,0,0,0.6)]
            p-5 overflow-y-auto lg:rounded-3xl"
              >
                <button
                ...
                <div className="mb-4 space-y-1">
                  ...
                  {topics.length === 0 && (
                    <p className="text-sm text-gray-400">No notes created</p>
                  )}

                  <ul className="space-y-3">
                    {topics.map((t, i) => (
                      <li
                        key={i} onClick={() => {openNotes(t._id)}}
                        className={`cursor-pointre roundedxl p-3 border transition-all ${
                          activeNodeId === t._id
        ...
        className="lg:col-span-3 rounded-2xl bg-white shadow-[0_15px_40px_rgba(0,0,0,0.15)] p-6 min-h-[75vh]">
          ...
          {!loading && !selectedNote && (
            <div className="h-full flex items-center justify-center text-gray-400">
              Select a topic form the sidebar
            </div>
          )}
